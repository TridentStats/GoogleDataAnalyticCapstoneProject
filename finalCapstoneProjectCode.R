...
# Make sure "CyclisticTripData" is in working directory before running
# this file
# Folder containing dataset analyzed can be found here:
# https://drive.google.com/drive/folders/1iR-s0aje4tqz2GScxkx0iEezvKLjl2OI?usp=drive_link
...
# Creates list of all CSV files in above sub-folder:
...
# "Creates "all_trips" data frame, which should combine the 12 original
...
# Also removes small amount of rows that have 'NA' value for 'member_casual' column.
all_trips_v3 <- all_trips_v2[!(all_trips_v2$rideable_type == "docked_bike" | all_trips_v2$secs_elapsed<180 | all_trips_v2$secs_elapsed>28800 | is.na(all_trips_v2$member_casual)),]
